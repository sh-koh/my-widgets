...
module MyWidgets.Panel (panel) where
...
panel :: Int -> IO Astal.Window
panel monitor = do
  window <-
    new
      Astal.Window
      [ #monitor := fromIntegral monitor,
        #anchor := [Astal.WindowAnchorTop, Astal.WindowAnchorRight, Astal.WindowAnchorBottom],
        #exclusivity := Astal.ExclusivityNormal,
        #marginTop := 0,
        #marginBottom := 0,
        #child :=> new Astal.Box []
      ]
